...
namespace teddy
{
    ...
    concept out_var_values =
        requires(Vars vs, index_t i, uint_t v) { vs[i] = v; };
    ...
    public:
        ...
        requires(is_bdd<Degree>)
        auto from_pla(
            pla_file const& file, fold_type foldType = fold_type::Tree
         ...
         */
        template<
            bin_op Op, std::random_access_iterator I, std::sentinel_for<I> S>
        ...
        auto satisfy_all(uint_t val, diagram_t d) const -> std::vector<Vars>;
        ...
        auto satisfy_all_g(uint_t val, diagram_t d, O out) const -> void;
    ...
    {
        inline auto
        default_or_fwd(std::size_t const n, std::vector<index_t>& is)
        {
    ...
    template<class Data, degree Degree, domain Domain>
    auto diagram_manager<Data, Degree, Domain>::constant(uint_t const v)
        -> diagram_t
    {
    ...
    template<class Data, degree Degree, domain Domain>
    auto diagram_manager<Data, Degree, Domain>::variable(index_t const i)
        -> diagram_t
    {
    ...
    template<class Foo>
    requires(is_bdd<Degree>)
    auto diagram_manager<Data, Degree, Domain>::variable_not(index_t const i)
    ...
    template<class Data, degree Degree, domain Domain>
    auto diagram_manager<Data, Degree, Domain>::operator()(index_t const i)
        -> diagram_t
    {
    ...
    template<std::output_iterator<teddy::uint_t> O>
    auto
    diagram_manager<Data, Degree, Domain>::to_vector_g(diagram_t d, O out) const
        -> void
    {
    ...
    template<expression_node Node>
    auto
    diagram_manager<Data, Degree, Domain>::from_expression_tree(Node const& root
    ) -> diagram_t
    {
    ...
    template<bin_op Op>
    auto
    diagram_manager<Data, Degree, Domain>::apply(diagram_t d1, diagram_t d2)
        -> diagram_t
    {
    ...
    template<class Data, degree Degree, domain Domain>
    template<bin_op Op, std::ranges::input_range R>
    auto diagram_manager<Data, Degree, Domain>::left_fold(R const& ds)
        -> diagram_t
    {
        return this->left_fold<Op>(
            std::ranges::begin(ds), std::ranges::end(ds)
        );
    }
    ...
    auto diagram_manager<Data, Degree, Domain>::satisfy_all_g(
        uint_t const val, diagram_t d, O out
    ) const -> void
    {
        if constexpr (domains::is_fixed<Domain>()())
        {
        ...
        auto go = [this, &xs, val,
                   out](auto& self, auto const l, auto const n) mutable
        {
            ...
            else
            {
                ...
                nodes_.for_each_son(
                    n,
                    [=, &xs, iv = uint_t {0}](auto const son) mutable
                    {
                        xs[index] = iv;
                        self(self, l + 1, son);
                        ++iv;
    ...
    auto diagram_manager<Data, Degree, Domain>::cofactor(
        diagram_t d, index_t const i, uint_t const v
    ) -> diagram_t
    {
    ...
    template<class Data, degree Degree, domain Domain>
    auto diagram_manager<Data, Degree, Domain>::dependency_set(diagram_t d
    ...
    template<class Data, degree Degree, domain Domain>
    auto diagram_manager<Data, Degree, Domain>::node_count() const
        -> std::size_t
    {
    ...
    template<class Data, degree Degree, domain Domain>
    auto diagram_manager<Data, Degree, Domain>::get_var_count() const
        -> std::size_t
    {
    ...
    template<class Data, degree Degree, domain Domain>
    auto diagram_manager<Data, Degree, Domain>::set_gc_ratio(double ratio)
        -> void
    {
        ...
        auto const go = [&, this](auto const self, auto const l, auto const r)
        {
            ...
            else
            {
                ...
                auto sons           = nodes_.make_sons(
                    ...
                    {
                        auto const fst =
                            lhsLevel == topLevel ? l->get_son(k) : l;
                        auto const snd =
