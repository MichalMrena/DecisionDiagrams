...
namespace teddy
{
    ...
    auto apply_cache<Data, D>::find(node_t* const l, node_t* const r) -> node_t*
    {
        ...
        auto const matches =
            entry.oid == oid and entry.lhs == l and entry.rhs == r;
    ...
    template<class Data, degree D>
    auto apply_cache<Data, D>::rm_unused() -> void
    {
        for (auto& e : entries_)
        {
            if (e.result)
    ...
    template<class Data, degree D>
    auto apply_cache<Data, D>::load_factor() const -> double
    {
    ...
    auto apply_cache<Data, D>::rehash(std::size_t const newCapacity) -> void
    {
        ...
        debug::out(
            "apply_cache: Load factor is ", this->load_factor(),
    ...
    ) const -> bool
    {
        return bucketIt_ == rhs.bucketIt_ and
               lastBucketIt_ == rhs.lastBucketIt_ and node_ == rhs.node_;
    }
    ...
    template<class BucketIt, class Data, degree D>
    auto unique_table_it<BucketIt, Data, D>::get_bucket() const -> BucketIt
    {
        return bucketIt_;
    }

    template<class BucketIt, class Data, degree D>
    auto unique_table_it<BucketIt, Data, D>::move_next() -> node_t*
    {
    ...
    template<class Data, degree D>
    auto unique_table<Data, D>::erase(iterator const it) -> iterator
    {
    ...
    template<class Data, degree D>
    auto unique_table<Data, D>::adjust_capacity(std::size_t const domain)
        -> void
    {
    ...
    template<class Data, degree D>
    auto unique_table<Data, D>::load_factor() const -> double
    {
        return static_cast<double>(size_) /
               static_cast<double>(this->capacity());
    }
