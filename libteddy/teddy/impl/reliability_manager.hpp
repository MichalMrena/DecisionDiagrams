...
#include <teddy/impl/diagram_manager.hpp>
#include <teddy/impl/dplds.hpp>
...
#ifdef LIBTEDDY_ARBITRARY_PRECISION
  template<class Ps>
  auto precise_availability (Ps const &ps, diagram_t const &diagram)
    -> mpfr::mpreal;
#endif
...
private:
  ...
  struct cache_entry_equals {
    auto operator() (dpld_cache_entry const &lhs, dpld_cache_entry const &rhs)
      const {
...
private:
  ...
  auto state_frequency_impl (node_memo<double> &memo, node_t *node, int32 state)
    -> double;
  ...
  template<class Ps>
  auto ntps_level_impl (node_memo<double> &memo, Ps const &probs, node_t *root)
    -> void;
};
...
auto reliability_manager<Degree, Domain>::precise_availability(
  ...
  node_memo<mpfr::mpreal> memo
    = details::map_memo<mpfr::mpreal, Degree>(this->get_node_count());
  ...
  node_t *const root = diagram.unsafe_get_root();
  nodes.traverse_post(root, [&ps, &memo, &nodes] (node_t *const node) mutable {
    if (node->is_terminal()) {
      return;
    }
    ...
    for (int32 k = 0; k < domain; ++k) {
      ...
      prob += *memo.find(son) * ps[as_uindex(nodeIndex)][as_uindex(k)];
    }
...
auto reliability_manager<Degree, Domain>::symbolic_availability(
  ...
  this->get_node_manager().for_each_terminal_node([&exprMap,
                                                   state] (node_t *const node) {
    symprobs::expression val(static_cast<int32>(node->get_value() >= state));
  ...
  this->get_node_manager().traverse_post(
    ...
    [this, &probs, &exprMap] (node_t *const node) mutable {
      if (not node->is_terminal()) {
        auto [it, isIn]
          = exprMap.emplace(std::make_pair(node, symprobs::expression(0.0)));
        assert(isIn);
        symprobs::expression &expr = it->second;
        int32 const nodeIndex      = node->get_index();
        int32 const domain = this->get_node_manager().get_domain(nodeIndex);
        for (int32 k = 0; k < domain; ++k) {
          node_t *const son = node->get_son(k);
...
auto reliability_manager<Degree, Domain>::dpld_impl(
  ...
  if (lhs->is_terminal() && rhs->is_terminal()) {
    ...
    result
      = this->get_node_manager().make_internal_node(topIndex, TEDDY_MOVE(sons));
  }
...
auto reliability_manager<Degree, Domain>::to_dpld_e(
  ...
  if (varLevel < rootLevel) {
    ...
    newRoot
      = this->get_node_manager().make_internal_node(varIndex, TEDDY_MOVE(sons));
    return diagram_t(newRoot);
  }
...
auto reliability_manager<Degree, Domain>::to_dpld_e_impl(
  ...
  node_t *const newNode
    = this->get_node_manager().make_internal_node(nodeIndex, TEDDY_MOVE(sons));
