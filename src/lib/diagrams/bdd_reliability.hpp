...
namespace mix::dd
{
    ...
    public:
        ...
        auto availability   (bdd_t&& f, std::vector<double> const& ps) -> double;
        auto unavailability (bdd_t&  f, std::vector<double> const& ps) -> double;
        auto unavailability (bdd_t&& f, std::vector<double> const& ps) -> double;
        ...
        auto birnbaum_importance  (bdd_t&  dpbd, std::vector<double> const& ps) -> double;
        auto birnbaum_importance  (bdd_t&& dpbd, std::vector<double> const& ps) -> double;
        auto birnbaum_importances (std::vector<bdd_t>&  dpbds, std::vector<double> const& ps) -> std::vector<double>;
        auto birnbaum_importances (std::vector<bdd_t>&& dpbds, std::vector<double> const& ps) -> std::vector<double>;

        auto criticality_importance  (bdd_t&  dpbd, std::vector<double> const& ps, double const U, index_t const i) -> double;
        auto criticality_importance  (bdd_t&& dpbd, std::vector<double> const& ps, double const U, index_t const i) -> double;
        auto criticality_importances (std::vector<bdd_t>&  dpbds, std::vector<double> const& ps, double const U)    -> std::vector<double>;
        auto criticality_importances (std::vector<bdd_t>&& dpbds, std::vector<double> const& ps, double const U)    -> std::vector<double>;

        auto fussell_vesely_importance  (bdd_t dpbd, std::vector<double> const& ps, double const U, index_t const i) -> double;
        auto fussell_vesely_importances (std::vector<bdd_t> dpbds, std::vector<double> const& ps, double const U)    -> std::vector<double>;
    ...
    auto bdd_reliability<VertexData, ArcData, Allocator>::availability
        (bdd_t& f, std::vector<double> const& ps) -> double
    {
        for (auto& vertex : f)
        {
            vertex.data = 0.0;
        ...
        for (auto& vertex : f)
        {
            if (! f.is_leaf(&vertex))
            {
                vertex.son(0)->data += vertex.data * (1 - ps.at(vertex.index));
                vertex.son(1)->data += vertex.data * (ps.at(vertex.index));
            }
        }

        return f.true_leaf()->data;
    }
    ...
    auto bdd_reliability<VertexData, ArcData, Allocator>::unavailability
        (bdd_t& f, std::vector<double> const& ps) -> double
    {
        return 1 - this->availability(f, ps);
    }
    ...
    auto bdd_reliability<VertexData, ArcData, Allocator>::birnbaum_importance
        (bdd_t& dpbd, std::vector<double> const& ps) -> double
    {
    ...
    auto bdd_reliability<VertexData, ArcData, Allocator>::birnbaum_importances
        (std::vector<bdd_t>& dpbds, std::vector<double> const& ps) -> std::vector<double>
    {
    ...
    {
        auto positions = std::vector<std::pair<vertex_t*, log_val_traits<2>::type>> {};
        ...
        (vertex_t* const low, vertex_t* const high, index_t const i, bool const mark) -> vertex_t*
    {
        ...
        auto const v = manager_.create(nextId_--, i, arr_t {arc_t {low}, arc_t {high}}, mark);
        memo_.emplace(key, v);
