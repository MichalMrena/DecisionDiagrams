...
// minmax_expr:
    ...
    auto generate_minmax_expression
        ...
        , std::size_t const termSize ) -> minmax_expr
    {
        assert(varCount > 0);
        auto const indexFrom = index_t(0);
        auto const indexTo = static_cast<index_t>(varCount - 1u);
        auto indexDst = std::uniform_int_distribution<index_t>(
            indexFrom,
            indexTo
        );
        ...
        , std::vector<uint_t> const& vs ) -> uint_t
    {
        ...
        return rs::max(expr.terms_ | rs::views::transform([&vs](auto const& is)
        {
...
// expr_node:
    ...
    expr_node::variable_t::variable_t
        (index_t const i) :
        i_ (i)
    {
    }

    expr_node::constant_t::constant_t
        (uint_t const c) :
        c_ (c)
    {
    }
    ...
    auto expr_node::get_left
        () const -> expr_node const&
    {
        return *std::get<operation_t>(data_).l_;
    }

    auto expr_node::get_right
        () const -> expr_node const&
    {
    ...
    auto generate_expression_tree
        ...
        , std::mt19937_64&  rngbranch ) -> std::unique_ptr<expr_node>
    {
