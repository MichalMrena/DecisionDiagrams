...
namespace teddy
{
    ...
    ) -> minmax_expr
    {
        ...
        auto indexDst =
            std::uniform_int_distribution<index_t>(indexFrom, indexTo);
    ...
    expr_node::expr_node(
        expr_node_operation, operation_type const o,
        std::unique_ptr<expr_node> l, std::unique_ptr<expr_node> r
    )
    ...
    auto generate_expression_tree(
        std::size_t varcount, std::mt19937_64& rngtype,
    ...
    ) -> std::unique_ptr<expr_node>
    {
        auto go = [&, i = 0u](auto& self, auto const n) mutable
        {
            ...
            else
            {
                ...
                auto const op =
                    p < 0.5 ? operation_type::Min : operation_type::Max;
